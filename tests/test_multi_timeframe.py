...
검증 항목:
  ...
  3. strategy_one 통합: metadata에 rsi_1h 노출
"""
...
def test_all_tf_overbought_penalty():
    """1D/1h 모두 RSI 80+ → 0.85 페널티."""
    result = compute_multi_tf_penalty({"1D": 85.0, "1h": 82.0})
    assert abs(result - 0.85) < 1e-6


def test_all_tf_oversold_penalty():
    """1D/1h 모두 RSI 20- → 0.85 페널티."""
    result = compute_multi_tf_penalty({"1D": 15.0, "1h": 18.0})
    assert abs(result - 0.85) < 1e-6


def test_no_penalty_when_mixed():
    """일부만 극단적이면 페널티 없음."""
    result = compute_multi_tf_penalty({"1D": 30.0, "1h": 85.0, "4h": 50.0})
...
def test_partial_none_uses_available_values():
    ...
    result = compute_multi_tf_penalty({"1D": 85.0, "1h": None, "4h": 90.0})
    assert abs(result - 0.85) < 1e-6
...
def test_rsi_not_summed_across_tf():
    ...
    동일 ticker 에 대해 1D 단독 스캔 score 와 1D+1h 스캔 score 를 비교.
    ...
    # 1D + 1h
    ctx_multi = _make_ctx_with_multi_tf("MR", df_1d, df_1h=df_1h)
...
def test_strategy_one_has_rsi_tf_metadata():
    """strategy_one metadata 에 rsi_1h 키 존재."""
    scenario = ScenarioBuilder.perfect_double_bottom(seed=42)
    df_1d = scenario.df.iloc[:33]
    df_1h = _make_normal_rsi_tf_df(df_1d)

    ctx = _make_ctx_with_multi_tf("MR", df_1d, df_1h=df_1h)
