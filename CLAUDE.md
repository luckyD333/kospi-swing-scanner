...
## Project Structure
- `cli.py` — CLI 진입점 (스캔 + Phase 2 의사결정 모드 `--interview`/`--decide`)
...
# Phase 2: 의사결정 (가중치 인터뷰 → ranking → Decision Journal)
python cli.py --interview                          # weights.yml 생성
python cli.py --decide --top-n 5                   # ranking + decision_topN.md
python cli.py --decide --select 005930,000660 --notes "메모"
```
...
## Verification
변경 후: `.venv/bin/python -m pytest backtest_engine/tests/ tests/ -q` 통과 필수. 542개 이상 통과해야 함.
...
weights.yml(가중치)와 `.cache/regime_analysis.json`(시장 국면)은 `--decide`/`market_regime` 의존 산출물.
