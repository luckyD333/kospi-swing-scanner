"""
scripts/tune_strategy_five.py — Strategy Five 파라미터 그리드 서치

각 파라미터 조합으로 Walk-forward 백테스트를 실행해 최적 임계값을 탐색한다.

사용:
    python scripts/tune_strategy_five.py
    python scripts/tune_strategy_five.py --step 3 --top 20
"""
...
def run_one(
    ...
    strategy = StrategyFiveBullFlag(config=cfg)
...
GRID = {
    "min_pole_pct":    [0.04, 0.05, 0.06, 0.07, 0.08],
    "vol_shrink_ratio": [0.70, 0.80, 0.90, 1.00],
    "tight_range_mult": [1.5, 2.0, 2.5, 3.0],
    "vol_confirm_ratio": [0.70, 0.80, 0.90, 1.00],  # vol_now >= avg_vol * ratio
}
...
def build_config(pole_pct: float, shrink: float, tight: float, vol_ratio: float) -> StrategyFiveConfig:
    return StrategyFiveConfig(
        pole_lookback=BASE["pole_lookback"],
        min_pole_pct=pole_pct,
        flag_bars=BASE["flag_bars"],
        vol_shrink_ratio=shrink,
        tight_range_mult=tight,
...
class PatchedStrategy(StrategyFiveBullFlag):
    """vol_confirm_ratio 를 적용하기 위해 scan() 을 패치."""
    def __init__(self, config: StrategyFiveConfig, vol_confirm_ratio: float):
        super().__init__(config=config)
        self._vol_confirm_ratio = vol_confirm_ratio

    def scan(self, ctx: ScanContext, top_n: int):
        candidates = super().scan(ctx, top_n=top_n * 3)
        # vol_confirm_ratio 필터: vol_now >= avg_vol * ratio
        # super().scan() 내부에서 이미 필터링됐으므로 metadata의 vol_ratio 기반 재필터
        filtered = [
            c for c in candidates
            if c.metadata.get("vol_ratio", 1.0) >= self._vol_confirm_ratio
        ]
...
def run_grid(
    ...
    for i, combo in enumerate(combos):
        ...
        cfg = build_config(
            pole_pct=params["min_pole_pct"],
            shrink=params["vol_shrink_ratio"],
            tight=params["tight_range_mult"],
            vol_ratio=params["vol_confirm_ratio"],
        )

        # vol_confirm_ratio 는 내부에서 처리 (scan override)
        strategy = PatchedStrategy(cfg, vol_confirm_ratio=params["vol_confirm_ratio"])
...
def print_results(results: list[RunResult], top_n: int) -> None:
    ...
    print(f"{'='*90}")
    print(f"  {'pole%':>6} {'shrink':>7} {'tight':>6} {'vol_r':>6} | {'신호':>5} {'승률':>7} {'avgPnL':>8} {'PF':>6} {'avgWin':>8} {'avgLoss':>8}")
    print(f"  {'-'*88}")

    for r in valid[:top_n]:
        p = r.params
        print(
            f"  {p['min_pole_pct']*100:>5.0f}%"
            f"  {p['vol_shrink_ratio']:>7.2f}"
            f"  {p['tight_range_mult']:>6.1f}"
            f"  {p['vol_confirm_ratio']:>6.2f}"
            f" | {r.signals:>5}"
            f"  {r.win_rate:>6.1f}%"
            f"  {r.avg_pnl:>+7.2f}%"
    ...
    baseline = next(
        (r for r in results
         ...
         and abs(r.params["vol_shrink_ratio"] - 0.70) < 0.001
         and abs(r.params["tight_range_mult"] - 1.5) < 0.001
         and abs(r.params["vol_confirm_ratio"] - 1.0) < 0.001),
        None,
    )
    if baseline:
        p = baseline.params
    ...
    if best:
        ...
        print(f"    vol_confirm_ratio = {p['vol_confirm_ratio']:.2f}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Strategy Five 파라미터 그리드 서치")
    ...
    print("Strategy Five — 파라미터 그리드 서치")
