...
리포 루트의 `weights.yml` 을 직접 편집하세요. (1~3일 단기 스윙 보유 환경 기준 표준 권장값이 커밋돼 있어요.)
...
`weights.yml` 디폴트에 `regime_score` priority 가 weight 10 으로 포함돼
...
- `false` — `dynamic_weights_error` 필드에 stderr 메시지 (최대 500자)
  - `weights.yml not found at ...` — `git restore weights.yml` 로 표준 권장값 복원 후 재실행
